Function Get-RebootHistory { 
 ...
 $d++
                ...
                Try { 
                    $RecentUnexpected = ($UnexpectedShutdowns | Where-Object { ((Get-Date)-(Get-Date -Date $_)).TotalDays -le 30 }).Count
                }
                Catch { 
                ...
                $ShutdownDetail | Select-Object -First 1 | ForEach-Object { 
                    New-Object -TypeName PSObject -Property @{
                        ...
                        LastShutdown        = (Get-Date -Date (([WMI]'').ConvertToDateTime($_.TimeGenerated)) -Format g)
                        RecentShutdowns     = ($BootHistory | Where-Object { ((Get-Date)-(Get-Date -Date $_)).TotalDays -le 30 }).Count
                        LastShutdownReason  = 'Reason Code: {0}, Reason: {1}' -f ($_.InsertionStrings[3], $_.InsertionStrings[2])
                    } | Select-Object -Property $BootInformation
                }
            }
            Catch { 
                ...
                If ($_.CategoryInfo.Activity -ne 'Get-Date') { 
                    Write-Warning -Message ("Unable to retrieve boot history for {0}. `nError Details: {1}" -f ($Computer, $_))
                }
            }
